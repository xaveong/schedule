...
if not df.empty:  
    for _, row in df.iterrows():  
        ...
        # [개선] 시작일부터 종료일까지 모든 날짜에 일정을 추가함  
        current_date = start_dt.date()  
        end_date = end_dt.date()  
          
        while current_date <= end_date:  
            if current_date.year == selected_year and current_date.month == selected_month:  
                day = current_date.day  
                if day not in schedules_by_day:  
                    schedules_by_day[day] = []  
                  
                # 표시 텍스트 최적화: 시작일인 경우에만 시간 표시, 나머지는 '계속' 또는 빈칸  
                time_str = start_dt.strftime("%H:%M") if current_date == start_dt.date() else ".."  
                  
                # 튜플 형태로 저장하여 렌더링 시 활용  
